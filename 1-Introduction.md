...
최근의 중요한 연구 동향으로는 단일세포체학(Single-cell omics)과 공간체학(Spatial omics)이 있다. 이 분야들은 세포 유형을 단일 세포 단위 또는 공간상에서 구분하는 연구로, 상피세포, 근육세포, 지방세포, 신경세포 등 다양한 세포 유형의 특성을 파악한다. Allen Brain Atlas, BRAIN Initiative Cell Census Network (BICCN), Human Cell Atlas 등 다양한 국제 컨소시엄에서 중요하게 다루는 주제이며, 특히 뇌과학 분야에서 이러한 연구가 활발히 진행되고 있다.

## 1.8 의생명정보학 데이터 분석을 위한 기초 도구

의생명정보학 연구를 위해서는 리눅스 환경에서의 작업이 필수적이다. 리눅스는 대용량 데이터 처리에 적합한 운영체제로, 다음과 같은 이유로 생물정보학 분야에서 널리 사용된다:
